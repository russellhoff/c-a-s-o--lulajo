...
namespace PracticaCaso {
	DsmServer::DsmServer(int p): nidCounter(-1), nodeCounter(0), TcpListener(p), lastNid(0) {
		//create lock modification 4
	...
	void DsmServer::dsm_exit(DsmNodeId nodeId) {
		...
		// Open read lock in modification 4
	...
	//MOD 4: JUST LAST NODE UPDATED CAN FREE
			...
			if (this->blockMetadataMap.find(blockId) != this->blockMetadataMap.end()) {
				...
				if (blockMetadata.lastAccessNode == nid) {
					...
					for (vector<DsmBlock>::iterator it = blocksRequested.begin(); it!=blocksRequested.end(); ++it) {
						if ( (blockMetadata.lastAccessNode == it->lastAccessNode) && (blockMetadata.blockSize == it->blockSize) && (blockMetadata.addr == it->addr)) {
